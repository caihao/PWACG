import os
import sys
import json
import logging
import logging.config

foo_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
os.chdir(foo_path)
sys.path.append(foo_path)

class Logger():
    def __init__(self,logger_name):
        self.logger = logging.getLogger(logger_name)
        with open("config/logconfig_select.json","r") as config:
            LOGGING_CONFIG = json.load(config)
            logging.config.dictConfig(LOGGING_CONFIG)

logger = Logger("select")

{% for codescript in jinja_draw_info.select.CodeScript %}
from rendered_scripts import {{codescript|replace('.py','')}} as select_object_{{loop.index0}}
{% endfor %}
args = select_object_0.args()
{% for key, value in run_config.items() -%}
args.{{key}} = {{value}}
{% endfor %}
{% for key, value in data_config.items() -%}
args.{{key}} = {{value}}
{% endfor %}

{% for codescript in jinja_draw_info.select.CodeScript %}
cl = select_object_{{loop.index0}}.Control(args)
cl.run_multiprocess()
{% endfor %}


os.system("cp output/fit/fit_result_{{generator_id}}/pwa_info* data/select/")